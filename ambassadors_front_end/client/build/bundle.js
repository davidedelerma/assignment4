	...
	var LoginBox = React.createClass({
	  ...
	  componentDidMount: function componentDidMount() {
	    ...
	    request.onload = function () {
	      if (request.status === 200) {
	        var receinvedTeacher = JSON.parse(request.responseText);
	        _this.setTeacher(receinvedTeacher);
	      } else if (request.status === 401) {
	        _this.setState({ currentTeacher: false });
	      }
	    };
	    request.send(null);
	  },
	  render: function render() {
	    var mainDiv = React.createElement(
	      ...
	      React.createElement(SignIn, { url: this.props.url + "teachers/sign_in.json",
	        onSignIn: this.setTeacher }),
	      React.createElement(SignUp, { url: this.props.url + "teachers.json", onSignUp: this.setTeacher })
	    );
	    if (this.state.currentTeacher) {
	      mainDiv = React.createElement(
	        ...
	        React.createElement(SignOut, { url: this.props.url + "teachers/sign_out.json", onSignOut: this.setTeacher })
	      );
	    }
	    return React.createElement(
	      'div',
	  ...
	  signIn: function signIn() {
	    ...
	    request.onload = function () {
	      if (request.status === 201) {
	        var teacher = JSON.parse(request.responseText);
	        _this.props.onSignIn(teacher);
	      }
	    };
	    var data = {
	      teacher: {
	    ...
	    return React.createElement(
	      ...
	      React.createElement(
	        ...
	        this.state.ambassadors.filter(function (ambassador) {
	          ...
	          return React.createElement(Ambassador, _extends({}, ambassador, { value: ambassador.id, key: ambassador.id, addToFavourites: _this.addToFavourites }));
	        })
	      )
	    );
	  }
